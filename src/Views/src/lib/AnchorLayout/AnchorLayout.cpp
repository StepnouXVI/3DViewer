...
AnchorLayout *AnchorLayout::get(QWidget *widget) {
  ...
  AnchorLayout *layout =
      widget->findChild<AnchorLayout *>("", Qt::FindDirectChildrenOnly);
  if (layout == nullptr) layout = new AnchorLayout(widget);
...
void AnchorLayout::centerIn(AnchorLayout *other) {
  if (!this->isAnchorAllowed(other)) return;

  if (m_leftLine) m_leftLine->anchorTo(nullptr);

  if (m_topLine) m_topLine->anchorTo(nullptr);

  if (m_rightLine) m_rightLine->anchorTo(nullptr);

  if (m_bottomLine) m_bottomLine->anchorTo(nullptr);

  if (other != nullptr) {
    this->horizontalCenter()->anchorTo(other->horizontalCenter());
    this->verticalCenter()->anchorTo(other->verticalCenter());
  } else {
    if (m_hcenterLine) m_hcenterLine->anchorTo(nullptr);

    if (m_vcenterLine) m_vcenterLine->anchorTo(nullptr);
  }
}

AnchorLayout *AnchorLayout::fill(AnchorLayout *other) {
  if (!this->isAnchorAllowed(other)) return this;
...
void AnchorLayout::setMargins(int margin) {
  if (m_margins == margin) return;

  if (m_leftLine) m_leftLine->setMargin(margin);

  if (m_topLine) m_topLine->setMargin(margin);

  if (m_rightLine) m_rightLine->setMargin(margin);

  if (m_bottomLine) m_bottomLine->setMargin(margin);

  m_margins = margin;
}

void AnchorLayout::update() { m_updateTimer.start(0, this); }
...
void AnchorLayout::timerEvent(QTimerEvent *te) {
  if (m_updateTimer.timerId() != te->timerId()) return;
  ...
  for (int i = 0; i < 6; i++) {
    if (lines[i]) lines[i]->update();
  }

  Q_FOREACH (AnchorLine *line, m_customLines) line->update();
}
...
AnchorLine::~AnchorLine() {
  if (m_anchoredTo != nullptr) m_anchoredTo->removeFromUpdateList(this);
  m_anchoredTo = nullptr;

  QList<AnchorLine *> updateList(m_updateList);
  m_updateList.clear();
  Q_FOREACH (AnchorLine *line, updateList) line->anchorTo(nullptr);
}

void AnchorLine::setOffset(int val) {
  if (m_offset == val) return;
...
void AnchorLine::setOffsetDirection(int dir) {
  if (m_edge != Horizontal && m_edge != Vertical) return;

  const int newdir = (dir < 0) ? -1 : 1;
  if (m_offsetDirection == newdir) return;
...
AnchorLine *AnchorLine::anchorTo(AnchorLine *line) {
  if (m_edge == Horizontal || m_edge == Vertical) return this;

  if (line == m_anchoredTo) return this;

  if (line != nullptr && this->layout() == line->layout()) return this;
  ...
  if (this->isVerticalLine() && !line->isVerticalLine()) return this;

  // paranoia check
  if (this->isHorizontalLine() && !line->isHorizontalLine()) return this;
...
void AnchorLine::addToUpdateList(AnchorLine *line) {
  if (line == nullptr || m_updateList.contains(line)) return;
...
void AnchorLine::update() {
  auto updateGeometry = [=]() {
    if (m_anchoredTo == nullptr) return;

    Relationship rel = relationship(this, m_anchoredTo);
    if (rel == NoRelationship) return;
...
void AnchorLine::updateList() {
  Q_FOREACH (AnchorLine *line, m_updateList) line->update();
}
...
AnchorLine::Relationship AnchorLine::relationship(const AnchorLine *line1,
  ...
  if (line1 == nullptr || line2 == nullptr) return NoRelationship;

  QWidget *w1 = line1->widget();
  QWidget *w2 = line2->widget();

  if (w2 == w1->parentWidget()) return ParentChildRelationship;

  if (w2->parentWidget() == w1->parentWidget()) return SiblingRelationship;
